...
String data;                                          // Mode ON = Music, 2 = Still, Anything else = off
String bluetooth;
int brightness;
...
void loop() 
{
  ...
  if (data == "OFF")
  {
    for (int i = 0; i < 7; i++) 
    {
    //Palatis::SoftPWM.set(pwmLEDsLeft[i], 0);
    //Palatis::SoftPWM.set(pwmLEDsRight[i], 0);
    }
  }
...
{                                                        // Read the seven spectrum bands from the MSGEQ7 chips
  ...
  for (int i = 0; i < 7; i++)                           // Cycle through the 7 spectrum bands
  {
    ...
    spectrumLeft[i] = analogRead(channelLeft) / 10;     // Store current values from left & right channels
    spectrumRight[i] = analogRead(channelRight) / 10;   // Divide 0-1023 by 10 to give about 0-100
